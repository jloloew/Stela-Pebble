...
#define JL_ENABLE_LOG_LEVEL_DEBUG_VERBOSE	(1 && JL_ENABLE_LOG_LEVEL_DEBUG)

#define JL_IF_DEBUG if (JL_DEBUG_ENABLED)

	// Prepend the file name.
#if JL_ADD_FILE_TO_LOGS
	#define JL_ADD_FILE2(level, fmt, ...) APP_LOG(level, "%s:" fmt, __FILE__ , ##__VA_ARGS__)
#else
	#define JL_ADD_FILE2(level, fmt, ...) APP_LOG(level, fmt , ##__VA_ARGS__)
#endif
...
#endif
#define JL_DEBUG_VERBOSE(fmt, ...) JL_DEBUG_VERBOSE2(fmt , ##__VA_ARGS__)
